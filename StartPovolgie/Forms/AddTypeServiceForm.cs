...
{
    public partial class AddTypeServiceForm : Form
    {
        private int id;
        TypeServiceController typeServiceController;

        public AddTypeServiceForm()
        {
            InitializeComponent();
            this.ActiveControl = textBox1;
            typeServiceController = new TypeServiceController();
        }

        public AddTypeServiceForm(int id, string nameTypeGood)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    if (id == 0)
                    {
                        TypeService typeService = new TypeService(textBox1.Text.Trim());
                        if (!typeServiceController.Insert(typeService))
                        {
                    ...
                    else
                    {
                        try
                        {
                            TypeService typeService = new TypeService(id, textBox1.Text.Trim());
                            if (!typeServiceController.Update(typeService))
                            {
